...
$ConfigFiles = @{
    "OPC UA Server" = "opcua_server\config\serversettings.env"
...
$CurrentOpcIp = Get-CurrentValue "opcua_server\config\serversettings.env" 'host:\s*([^\s\r\n]+)'
$CurrentOpcPort = Get-CurrentValue "opcua_server\config\serversettings.env" 'port:\s*(\d+)'
...
Write-Host "  [1/4] opcua_server\config\serversettings.env..." -ForegroundColor Cyan
$opcContent = Get-Content "opcua_server\config\serversettings.env" -Raw
...
$opcContent | Set-Content "opcua_server\config\serversettings.env" -NoNewline
