#!/bin/bash
# copy_portable.sh — Assembles the Linux portable deployment folder.
# Run from the project root after cargo build --release + npm run build.
# A fresh QuestDB is always downloaded, so historian data is never polluted.
...
# --- Helper: copy directory with existence check ---
copy_dir() {
    local SRC="$1"
    local DST="$2"
    if [ -d "$SRC" ]; then
...
#!/bin/bash
# Start AuTHRity — opens each service in its own terminal window
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
cd "$DIR"
mkdir -p logs

# Open CMD in a new terminal window; falls back to background log if no GUI terminal found
launch_term() {
    local TITLE="$1"
    local WORKDIR="$2"
    local CMD="$3"
